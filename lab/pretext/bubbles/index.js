...
var PAGE_MAX_WIDTH = 1080;
var DESKTOP_PAGE_MARGIN = 32;
var MOBILE_PAGE_MARGIN = 20;
var GRID_GAP = 16;
var PANEL_PADDING_X = 36;
function prepareBubbleTexts(texts) {
  return texts.map((text) => ({
    prepared: prepareWithSegments(text, FONT)
  }));
}
function getMaxChatWidth(minWidth, viewportWidth) {
...
function collectWrapMetrics(prepared, maxWidth) {
  let maxLineWidth = 0;
...
function computeBubbleRender(preparedBubbles2, chatWidth) {
  ...
  const widths = [];
  for (let index = 0; index < preparedBubbles2.length; index++) {
    const bubble = preparedBubbles2[index];
...
var shrinkNodes = getChatMessageNodes(domCache.chatShrink);
var preparedBubbles = prepareBubbleTexts(shrinkNodes.map(readNodeText));
var st = {
...
function getInitialChatWidth() {
  ...
  const parsed = datasetValue === void 0 ? Number.NaN : Number.parseInt(datasetValue, 10);
  if (Number.isFinite(parsed)) return parsed;
  return Number.parseInt(domCache.slider.value, 10);
}
function getChatMessageNodes(chat) {
